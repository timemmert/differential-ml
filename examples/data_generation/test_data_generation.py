...
from examples.data_generation.functions import OutputFormat, quadratic_one_in_one_out, quadratic_two_in_one_out, \
    polynomial_trigonometric_two_in_one_out, trigonometric_two_in_two_out
...
class TestRandomUtil(unittest.TestCase):
    ...
    def test_quadratic_one_dim(self):
        n_test = 1000
        a = 0.1
        data, y, dydx = quadratic_one_in_one_out(a, n_test, output_format=OutputFormat.TORCH)
    ...
    def test_quadratic_two_dim(self):
        n_test = 10
        a = 0.1
        data, y, dydx = quadratic_two_in_one_out(a, n_test, output_format=OutputFormat.TORCH)
    ...
    def test_poly_trigonometric_two_dim(self):
        ...
        data, y, dydx = polynomial_trigonometric_two_in_one_out(n_test, output_format=OutputFormat.TORCH)
    ...
    def test_trigonometric_two_in_two_out(self):
        n_test = 1000
        data, y, dydx = trigonometric_two_in_two_out(n_test, output_format=OutputFormat.TORCH)

        y_manual = torch.stack(
            (
                torch.sin(data[:, 0]), torch.cos(data[:, 1])
            ),
            dim=1
        )
        dydx_manual = torch.stack(
            (
                torch.stack(
                    (torch.cos(data[:, 0]), torch.zeros(n_test)), dim=1
                ),
